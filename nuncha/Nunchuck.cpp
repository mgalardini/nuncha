...
uint8_t NunchuckInterface::begin()
{
  ...
  twowire_beginTransmission( NCDevID );
  twowire_send( (uint8_t) 0xF0 );
  twowire_send( (uint8_t) 0xAA );
  wireErr = twowire_endTransmission();
  if ( wireErr ) return ( wireErr );
  
  // write crypto key ( two 6 block writes, one 4 block write )
  twowire_beginTransmission( NCDevID );
  twowire_send( (uint8_t) 0x40 );
  for ( byteCnt = 0; byteCnt < 6; byteCnt++ ) twowire_send( (uint8_t) 0x00 );
  wireErr = twowire_endTransmission();
  if ( wireErr ) return ( wireErr );
    
  twowire_beginTransmission( NCDevID );
  twowire_send( (uint8_t) 0x40 );
  for ( byteCnt = 6; byteCnt < 12; byteCnt++ ) twowire_send( (uint8_t) 0x00 );
  wireErr = twowire_endTransmission();
  if ( wireErr ) return ( wireErr );
  
  twowire_beginTransmission( NCDevID );
  twowire_send( (uint8_t) 0x40 );
  for ( byteCnt = 12; byteCnt < 16; byteCnt++ ) twowire_send( (uint8_t) 0x00 );
  wireErr = twowire_endTransmission();
  if ( wireErr ) return ( wireErr );
   
  // send a zero to the device to resample and store in register for next poll
  twowire_beginTransmission( NCDevID );
  twowire_send( (uint8_t) 0x00 );
  wireErr = twowire_endTransmission();
  if ( wireErr ) return ( wireErr );
